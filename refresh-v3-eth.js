// refresh-v3-eth.js — pulls VCL Clarity V3 — ETH (Sweep+BoS) from Notion and writes v3-eth-data.json
// Extracts RAW inputs only; the scoreboard (v3-eth.html) recomputes R client-side using the
// corrected full-ladder model. Requires env NOTION_TOKEN. Run by the Actions cron loop.
const fs = require('fs');
const path = require('path');

const NOTION_TOKEN = process.env.NOTION_TOKEN;
const V3_DB = '17736d193e324254b76cbf9054b89184'; // VCL Clarity V3 — ETH Trade Log
const OUT = path.join(__dirname, 'v3-eth-data.json');

if (!NOTION_TOKEN) { console.error('refresh-v3-eth: NOTION_TOKEN missing'); process.exit(1); }
...
(async () => {
  try {
    ...
    console.log(`refresh-v3-eth: wrote ${data.length} trades to v3-eth-data.json`);
  } catch (e) {
    console.error('refresh-v3-eth failed:', e.message);
    process.exit(1);
  }
})();
